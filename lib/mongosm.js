...
function log() {
  ...
  console.log("Entry parsed: "+entryParsed);
  console.log("Entry saved: "+entrySaved);
  if(eof && entryParsed == entrySaved){
...
//funzioni importate da queue
  ...
  function upsert () {
    ...
    for (var atr in value) {
      if (!!options[this.constructor.modelName] && options[this.constructor.modelName].keepAttributes.indexOf(atr) < 0){
        this.set(atr, undefined);
      }
    }
    Model.findOneAndUpdate(query, this.toObject(), {upsert: true}, saveCB);
  }
  ...
  function populateRelation (relation, cb) {
    ...
    if (!!typeList.way && !!typeList.relation) {
      entrySaved++;
      return cb();
    }

    if(typeList.way == undefined){
      entrySaved++; //in caso di relation non salvata incremento comunque il counter
      return cb();
    }
    var Way = relation.constructor.base.models.way;
    var query = {};
    query[idType] = {$in: typeList.way};
    Way.find(query)
